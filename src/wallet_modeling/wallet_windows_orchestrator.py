...
class MultiWindowOrchestrator:
    ...
    def _merge_window_training_data(self) -> pd.DataFrame:
        """
        ...
        """
        if not self.window_training_data_filepaths:
            raise ValueError("No window training data files found. Run generate_windows_training_data first.")

        logger.info("Beginning merge of %d window training datasets...",
                    len(self.window_training_data_filepaths))
        ...
        date_file_map = dict(zip(window_dates, self.window_training_data_filepaths.values()))
