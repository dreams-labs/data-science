...
class RegressionEvaluator:
    ...
    def get_summary_report(self):
        ...
        RMSE: {self.metrics['rmse']:.3f}
        MAE: {self.metrics['mae']:.3f}
        MAPE: {self.metrics['mape']:.1f}%

        Residuals Analysis:
        ------------------
        Mean of Residuals: {self.metrics['residuals_mean']:.3f}
        Standard Deviation of Residuals: {self.metrics['residuals_std']:.3f}
        95% Prediction Interval: ±{self.metrics['prediction_interval_95']:.3f}
        """
    ...
    def plot_evaluation(self, plot_type='all'):
        ...
        if plot_type == 'all':
            ...
            self._plot_actual_vs_predicted(axes[0, 0])
            self._plot_residuals(axes[0, 1])
            self._plot_residuals_distribution(axes[1, 0])
    ...
    def _plot_residuals(self, ax):
        ...
        ax.scatter(self.y_pred, self.residuals, alpha=0.5)
    ...
    def _plot_feature_importance(self, ax):
        ...
        if 'importances' in self.metrics:
            sns.barplot(
                data=self.metrics['importances'].head(20),
                x='Importance',
                y='Feature',
                ax=ax
            )
