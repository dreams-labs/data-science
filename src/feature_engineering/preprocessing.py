...
def preprocess_sets_X_y(
    ...
    """
    Wrapper function that preprocesses the 'X' data in each set using DataPreprocessor and returns
    the updated dictionary.

    Args:
        sets_X_y_dict (Dict[str, Tuple[pd.DataFrame, pd.Series]]): Dictionary with keys for each
            set type (e.g., 'train', 'test'), and values are tuples containing (X, y) data. This
            is generated by data_splitting.perform_train_test_validation_future_splits().
        config (Dict[str, Any]): Configuration dictionary for DataPreprocessor.
        metrics_config (Dict[str, Any]): Metrics configuration for DataPreprocessor.
        modeling_config (Dict[str, Any]): Modeling configuration for DataPreprocessor.

    Returns:
        Dict[str, Tuple[pd.DataFrame, pd.Series]]: Updated dictionary with preprocessed 'X' data.

    Raises:
        ValueError: If any of the datasets have an empty 'X' DataFrame.
    """
    # Instantiate the DataPreprocessor
    preprocessor = DataPreprocessor(config, metrics_config, modeling_config)

    # Prepare datasets for preprocessing
    datasets_for_preprocessing = {}

    for set_key, data_tuple in sets_X_y_dict.items():
        # Unpack the tuple to get X and y
        X_data, y_data = data_tuple
        if X_data.empty:
            # Raise an error if the set is empty
            raise ValueError(
                f"The dataset '{set_key}' has no records and cannot be preprocessed."
            )
        else:
            # Add the dataset to the preprocessing dictionary
            datasets_for_preprocessing[set_key] = X_data

    # Preprocess the datasets
    preprocessed_X_datasets = preprocessor.preprocess(datasets_for_preprocessing)

    # Construct the preprocessed_sets_X_y_dict
    preprocessed_sets_X_y_dict = {}
